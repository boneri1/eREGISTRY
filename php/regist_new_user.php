...
if (isset($_POST['ok'])) {
	...
	if(mysqli_num_rows($select)>0){
		echo "The email is already registed";
	}
	else{
    // $hashedPassword = password_hash($password, PASSWORD_DEFAULT);


    $insert = mysqli_query($conn, "INSERT INTO users (FirstName,LastName,Password,ContactInfo,Email,NationalId,Address)VALUES('$firstName', '$lastName', '$password', '$phoneNumber', '$email', '$nationalId', '$address')");
    if ($insert) {
